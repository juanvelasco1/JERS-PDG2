...
import { Zap, DollarSign, Bot, Target } from "lucide-react";
...
const speedTicks = [
  { label: "4–6 meses", pct: 13 },
  { label: "4–8 semanas", pct: 50 },
  { label: "Inmediato", pct: 86 },
];

// x=velocidad (0=lento → 100=rápido), y=precio (0=barato → 100=caro)
const points: { name: string; note: string; x: number; y: number; highlight: boolean; labelAnchor: string }[] = [
  { name: "Agencia tradicional", note: "$10K–25K · 4–6 meses", x: 13, y: 90, highlight: false, labelAnchor: "above-right" },
  { name: "Boutique UX", note: "$5K–15K · 2–3 meses", x: 30, y: 76, highlight: false, labelAnchor: "above-right" },
  { name: "Freelancers", note: "$500–2K · 4–8 semanas", x: 50, y: 27, highlight: false, labelAnchor: "below-right" },
  { name: "Plantillas", note: "$100–300 · instantáneo", x: 87, y: 10, highlight: false, labelAnchor: "above-left" },
  { name: "JERS", note: "$1K–3K · 2–4 semanas", x: 80, y: 36, highlight: true, labelAnchor: "right" },
];

const reasons = [
  { Icon: Zap, text: "Entrega en 2–4 semanas con metodología ágil" },
  { Icon: Bot, text: "Diagnóstico automatizado acelera el proceso" },
  { Icon: DollarSign, text: "Precio de freelancer: $1,000–$3,000" },
  { Icon: Target, text: "Solución a medida, no plantilla genérica" },
];
...
export default function Slide10({ subStep = 0 }: { subStep?: number }) {
  return (
    <div className="bg-[#fcfaf5] relative size-full px-[108px] pt-[68px]">
      ...
      <div className="relative">
        ...
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 0.5 }} transition={{ delay: 0.28 }}
          ...
          Tan rápido como una plantilla, con la calidad de una agencia boutique.
        </motion.p>
      </div>
        ...
        <div className="relative shrink-0" style={{ width: `${AXIS_LEFT + PLOT_W + 20}px`, height: `${PLOT_H + 48}px` }}>
          ...
          <div className="absolute" style={{ left: `${AXIS_LEFT}px`, top: 0, width: `${PLOT_W}px`, height: `${PLOT_H}px` }}>
            ...
            <motion.div className="absolute pointer-events-none"
              style={{
                left: `${(80 / 100) * PLOT_W - 80}px`,
                top: `${PLOT_H - (36 / 100) * PLOT_H - 80}px`,
              }}
          ...
          <div className="bg-white rounded-[10px] border border-[rgba(29,78,216,0.12)] px-[22px] py-[20px]">
            ...
            <div className="mt-[16px] pt-[14px] border-t border-[rgba(90,78,55,0.08)]">
              <p className="font-['Google_Sans:Regular',sans-serif] text-[12px] text-[#0b1422] opacity-45 leading-[17px] italic">
                "Deja de gastar en parches temporales. Invierte en el crecimiento de tu negocio."
              </p>
            </div>
          </div>
        </motion.div>
